...
export function Dashboard() {
  ...
  const createWishlist = async (e) => {
    e.preventDefault();
    if (newWishlistName.trim() === '' || (enableEvent && !eventDate)) {
    ...
    if (user) {
      const { data, error } = await supabase
        ...
        .insert({ 
          ...
          // **NEW**: Conditionally add event_date
          event_date: enableEvent ? eventDate : null,
        })
        .select();
      ...
      <div className="bg-white/10 backdrop-blur-md p-6 sm:p-8 rounded-2xl shadow-2xl border border-white/20">
        ...
        <form onSubmit={createWishlist} className="space-y-4 mb-10">
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            />
            <button type="submit" className="flex-shrink-0 px-5 py-3 font-semibold text-white bg-brand-600 hover:bg-brand-700 rounded-lg transition-colors flex items-center justify-center gap-2">
              <Plus size={20} />
          ...
          <div className="flex flex-col sm:flex-row gap-4 items-center">
            <div className="flex items-center">
                <input 
                    ...
                    className="h-4 w-4 rounded border-gray-500 bg-transparent text-brand-600 focus:ring-brand-500"
                />
                <label htmlFor="enableEvent" className="ml-2 text-gray-200">Enable Event</label>
            </div>
            {enableEvent && (
                 <input
                    type="date"
                    ...
                    required
                    className="flex-grow w-full sm:w-auto px-4 py-2 bg-white/10 text-white border border-white/20 rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-500"
                 />
            )}
          </div>
        </form>
          ...
          <div className="space-y-3">
            {wishlists.length > 0 ? (
              wishlists.map((wishlist) => (
                ...
                >
                  {/* **NEW**: Event Date Badge */}
                  {wishlist.event_date && (
                    <div className="absolute top-2 right-2 flex items-center gap-1 text-xs bg-brand-600 text-white px-2 py-1 rounded-full">
                        <Calendar size={12} />
